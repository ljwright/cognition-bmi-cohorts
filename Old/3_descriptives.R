...
plot_dens(FALSE)
ggsave("Images/dens_cog.png", width = 29.7, 
       height = 21, dpi = 600, units = "cm")


plot_dens(TRUE)
ggsave("Images/dens_height.png", width = 29.7, 
       height = 21, dpi = 600, units = "cm")
...
ggplot(df_chart) +
  ...
  theme(strip.placement = "outside",
        strip.text.y.left = element_text(angle = 0),
        strip.background.y = element_blank())
ggsave("Images/dens_height_chart.png", width = 29.7, 
       height = 21, dpi = 600, units = "cm")
...
get_correlation <- function(df){
  ...
  get_boot <- function(boot, df_pair){
    set.seed(boot)
  ...
  get_ci <- function(x, y){
    ...
    cors <- map_dbl(1:200, get_boot, df_pair) 
...
cors$var_dict <- c(height_11 = "Height @ Age 11", height_16 = "Height @ Age 16",
...
cors$df_clean <- cors$df %>%
  ...
  filter(x != "comp_resid_16", y != "comp_resid_16",
         x_clean < y_clean, str_length(cohort) == 5)
...
ggsave("Images/corr_all.png", height = 21,
       width = 29.7, units = "cm", dpi = 300)
...
cors$df_clean %>%
  filter(str_detect(x, "_height"),
         str_detect(y, "_height")) %>%
  mutate(across(beta:uci, round, 2),
         string = glue("{beta} (95% CI = {lci}, {uci}) in the {cohort}")) %>%
  pull(string) %>%
  glue_collapse(", ")
